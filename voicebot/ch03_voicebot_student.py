...
def STT(audio):
    filename='input.mp3'
    audio.export(filename, format="mp3") #음원 파일 열기
...
def main():
    ...
    # 기본 설명
    with st.expander("EY Bōt", expanded=True):
        st.write(
            '''
            - 음성비서 프로그램의 UI는 스트림릿을 활용했습니다.
            - STT(Speech-To-Text)는 OpenAI의 Whisper AI를 활용했습니다.
            - 답변은 OpenAI의 GPT 모델을 활용했습니다.
            - TTS (Text-To-Speech) 72 Google Translate TTS
    ...
    with st.sidebar:
        ...
        #Open AI API 키 입력받기
        openai.api_key = st.text_input(label="OPENAI API |", placeholder="Enter Your API Key", value="", type="password")

        st.markdown("---")

        # GPT 모델을 선택하기 위한 라디오 버튼 생성
        model=st.radio(label="GPT",options=["gpt-4", "gpt-3.5-turbo"])

        st.markdown("---")

    # 리셋 버튼 생성
    if st.button(label="7|3|"): 
        #리셋 코드
        st.session_state["chat"] = []
